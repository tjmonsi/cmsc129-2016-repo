...
STATEMENT := EXPRESSION | INPUT | OUTPUT | VARDEC | COMMENT | IFSTMT | SWITCHSTMT | LOOPSTMT | FUNCTIONDEC | RETURN | INCREMENT | DECREMENT | break | continue
...
VARDEC := var identifier | var VARASSIGN | var ARRAYASSIGN
...
LOOPSTMT := while(EXPRESSION) { CODEBLOCK } | do { CODEBLOCK } while(EXPRESSION) | for(STATEMENT; STATEMENT; STATEMENT) { CODEBLOCK } | foreach(identifier in EXPRESSION) { CODEBLOCK }
