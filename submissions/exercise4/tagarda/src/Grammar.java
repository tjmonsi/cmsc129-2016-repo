...
class Grammar {
    ...
    static MessageHandler CheckGrammar(Grammar start, int tokenindex,int tabcount) {
        ...
        for(GrammarConstruct[] gram: start.g){
            ...
            mh.actionlist = new ArrayList<Action>();
                ...
                try{
                    ...
                    if(gram[i].type == 1){
                        ...
                        if(gram[i].construct.trim().equals(temptoken.token.trim())){
                            tempaction = new Action(start.name, temptoken.token, temptoken.value);
                            mh.actionlist.add(tempaction);
                        ...
                        if(mhtemp.isFailed == true){
                            ...
                            for (Action x:mhtemp.actionlist){
                                mh.actionlist.add(x);
                            }
                        }
