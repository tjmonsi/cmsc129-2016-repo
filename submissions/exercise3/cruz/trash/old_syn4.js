...
function findNext(stack, target){

//Add Re-rule which finds a better prod rule

	var candidates = [];
	var results = [];
	if(stack[0] == gStart["root"]){	//when starting at root
		for (var i = 0; i < gRules.length; i++) {
			if(gRules[i].root == stack[0])
...
function check(lexemes){
	...
	var ruleStacks = [gStart["root"]];
	var i = 0;
	while(neolexes.length > 0){
		var tok = neolexes.shift();
		console.log(tok);
		var result = findNext(ruleStacks, tok["type"]);
