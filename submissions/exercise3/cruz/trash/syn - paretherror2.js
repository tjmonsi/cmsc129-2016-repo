...
//		console.log("ER",getRuleE(rule[i]))
...
function getNextNode(occurrence, index){
	...
	//Find the nearest unexpanded node/unvalued terminal
	// var stack = [];
	// var next = [pStart];
	var next = lastpos;
	if(index == undefined)
	...
	if(next.rule.root == "S"){
		console.log("ERROR--- REACHED START")
		return null;
	}
...
function check(lexemes){
	...
	for (var i = 0; i < lexemes.length; i++) {
		...
		var result = expandFor(lex.type, lex.token, 0);
