...
function findNext(current, target){
...
function check(lexemes){
	...
	while(neolexes.length > 0){
		...
		console.log(tok);
		var result = findNext(current, tok["type"]);
		var ruleStack = {"stack":[], "complete":false};
		for(var j = 0; j < result.length; j++){
			ruleStack["stack"].push(result[j]);
		}

		ruleStacks.push(ruleStack);

		for(var j = 0; j < result.length; j++){
			ruleStacks = validateStacks(ruleStacks);
		}
		
		console.log(ruleStacks[i]["stack"]);
...
//validates matches on previous rules
function validateStacks(ruleStacks){
	...
	for (var i = 0; i < ruleStacks.length; i++) {
		...
		//Form "stack groups" by checking previous rules
		//stack group = prod rules

		//Check "stack groups" by using LCPR
		//complete rule if only 1 stack rule remaining

		//If no matching "stack groups", syntax error
