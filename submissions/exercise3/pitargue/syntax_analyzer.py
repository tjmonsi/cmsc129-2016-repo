...
class Parser():
    ...
    def literal(self):
        if self.check_next(['STRING_LITERAL', 'INTEGER_LITERAL', 'FLOAT_LITERAL', 'TRUE_KEYWORD', 'FALSE_KEYWORD']):
            self.nextLexeme()
        else:
            print('expected literal before ' + self.lookahead[1])
    ...
    def expression(self):
        ...
        if self.check_next(['STRING_LITERAL', 'INTEGER_LITERAL', 'FLOAT_LITERAL', 'TRUE_KEYWORD', 'FALSE_KEYWORD']):
            ...
            self.nextLexeme()
        if self.check_next(['CLOSE_PARENTHESIS_KEYWORD']) and must_close:
            self.nextLexeme()
        if self.check_next(['PLUS_KEYWORD', 'MINUS_KEYWORD', 'MULTIPLY_KEYWORD', 'DIVIDE_KEYWORD', 'MODULO_KEYWORD']):
            self.nextLexeme()
            self.expression()
        elif self.check_next(['AND_KEYWORD', 'OR_KEYWORD', 'EQUALS_KEYWORD', 'NOT_EQUALS_KEYWORD', 'LESS_THAN_KEYWORD', 'LESS_THAN_OR_EQUALS_KEYWORD', 'GREATER_THAN_KEYWORD', 'GREATER_THAN_OR_EQUALS_KEYWORD']):
            self.nextLexeme()
            self.expression()
        elif self.check_next(['QUESTION_MARK_KEYWORD']):
            self.nextLexeme()
            self.expression()
